...
if [[ -d "/Applications/Atom.app/" ]]; then
   ...
   while true
   do
      warn "Install Atom?" true
      read -p "${BG_YELLOW}${BLACK}${BOLD}>>>>  [y]es or [n]o. ${RESET}"  -n 1 -r ATOM_RESPONSE

      case $ATOM_RESPONSE in
         [yY]* )

            inform "Downloading Atom..." true
            ...
            if [[ -e "$ATOM_INSTALLER" ]]; then
               inform "Removing $ATOM_INSTALLER..."

               rm -f "$ATOM_INSTALLER"

               if [[ -e "$ATOM_INSTALLER" ]]; then
                  warn "Removing $ATOM_INSTALLER was not successful. Please remove manually."
               else
                  inform "Atom’s installer removed successfully."
               fi
            else
         ...
         * )
            echo "${BG_YELLOW}${BLACK}${BOLD}>>>>  Please choose. Install Atom? [y]es, [n]o, or [q]uit. ${RESET} ";;
      esac
   done
fi
